...
7. 点击 `History` 可像 `Detail` 一样在主界面图表区域切换到历史模式，再次点击或按历史视图右上角 `×` 返回实时图表；历史日期可直接输入或用左右箭头切换，图表复用实时图表的颜色、曲线、坐标与 1h／2h／3h／6h／12h／24h／48h 滚轮档位。左键框选可局部放大，滚轮切换档位时自动取消框选；48h 会自动读取前一天数据。
